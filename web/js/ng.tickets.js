(function(){
    ...
    $(document).on('ready', function() {
        ...
        var forthDateWidget = getDateField($('#forth-date')[0], function(date) {
            var currentChoice = forthDateWidget.getDate();
            if (currentChoice.getTime() > backDateWidget.getDate().getTime()) {
                backDateWidget.setDate(new Date(currentChoice.getTime()+24*60*60*1000));
            }
        });
        var backDateWidget = getDateField($backDate[0], function(date){
            var currentChoice = backDateWidget.getDate();
            if (currentChoice.getTime() < forthDateWidget.getDate().getTime()) {
        ...
        var lastRequestedStartCity = null;
        var lastRequestedEndCity = null;
        var lastBothSearch = false;
        var currentForthTrip = null;
        var currentBackTrip = null;
        var isBackChanged = false;
        var chosenSeats = {forth:[], back:[]}; //seatIds

        $('#search-tr').on('click', function(event) {
            ...
            lastRequestedStartCity = dataStore.get(startCitySelectId);
            lastRequestedEndCity = dataStore.get(endCitySelectId);
            lastBothSearch = both;
            var data = {
                startCityId:startCitySelectId,
                endCityId:endCitySelectId,
                startDate:forthDateWidget.getDate().getTime(),
                tzOffset:new Date().getTimezoneOffset()/60*(-1)};
            if (both) data['endDate'] = backDateWidget.getDate().getTime();
            ...
            new Request('search', data).send(function(data) {
                ...
                if (lastBothSearch && !data['backTrips']) lastBothSearch = false;

                $forthDir.empty();
                var tripStr = lastRequestedStartCity.name + ' - ' + lastRequestedEndCity.name;
                ...
                for (var i = 0, size = responseForthTrips.length; i < size; i++) {
                    ...
                    trip.isForth = true;
                    $trips.append($(uc.tripTemplate(trip.id, trip['routeFirstCity'], trip['routeEndCity'],
                                        trip['startDate'], trip['endDate'], trip['seats'].length, trip.isForth)));
                ...
                if (data['backTrips']) {
                    $backDir.empty();
                    var backTripStr = lastRequestedEndCity.name + ' - ' + lastRequestedStartCity.name;
                    ...
                    for (var i = 0, size = responseBackTrips.length; i < size; i++) {
                        ...
                        trip.isForth = false;
                        $backTrips.append($(uc.tripTemplate(trip.id, trip['routeFirstCity'], trip['routeEndCity'],
                                        trip['startDate'], trip['endDate'], trip['seats'].length, trip.isForth)));
                        dataStore.set(trip, 'Trip');
                    }
                    $backDir.show();
                ...
                if (!isResultsBoardClickHandlerSet) {
                    ...
                    $forthDir.on('click', function(event) {
                        ...
                        if (!$forthTripEl.hasClass('active')) $forthTripEl.addClass('active');
                        ...
                        var forthTripCities = lastRequestedStartCity.name+' - '+lastRequestedEndCity.name;
                        var backTripCities = currentBackTrip? lastRequestedEndCity.name + ' - ' + lastRequestedStartCity.name : null;
        ...
        function showBusSeatsPopup(targetButton, tripId, allSeats, lockedSeats) {
            ...
            var popup = new Popup('Схема автобуса', setraBus, 'popup-bus black', function(popupId){
        ...
        function setReserveTicketHandler() {
            $('#reserve-ticket').on('click', function(event) {
                if (!isAuthorized()) {
                    new Message('Для здійснення операції авторизуйтесь, будь ласка', 10000);
                    return false;
                }
                ...
                new Request('order', {order: JSON.stringify(order)}).send(function(data) {
                    ...
                    if (data && data['order_id']) {
                        ...
                        var p = new Popup('Квитки заброньовано', '<div>Ви успішно забронювали квитки, підтвердження бронювання надіслано на вашу пошту</div><p><button id="reserve-ready" class="btn btn-green width-100">OK</button></p>', 'white', function(){
        ...
        function createOrder() {
            ...
            for (var i = 0; i < tickets.length; i++) {
                ...
                ticket['startCity'] = lastRequestedStartCity.id;
                ticket['endCity'] = lastRequestedEndCity.id;
    ...
    (function(){
        ...
        window.uc.tripTemplate = function(tripId, startCity, endCity, startDate, endDate, seatCount, isForth) {
            ...
            var start =
                '<div id="ticket'+ticketsCount+'" class="' +clazz+ '" style="z-index:'+(ticketsCount+5)+';">'+
                    '<div class="ticket-row">'+
                        ...
                        '<div class="price">'+
                            '<div class="price-disc"><span class="sign">₴</span><span class="price-self">'+parseInt(resultPrice*EURUAH)+'</span></div>'+
                            '<div class="price-full '+(discPrice?'' : 'hide')+'"><span class="price-self">'+(discPrice?parseInt(price*EURUAH) : '')+'</span></div>'+
                        '</div>'+
                    '</div>'+
                    '<div class="ticket-row">' +
