(function(){
    ...
    window.ActionDataProvider = function(action){
        ActionDataProvider.__super__.constructor.apply(this, arguments);
        this.action = action;
    };
    OOP.__extends(ActionDataProvider, DataProvider);

    ActionDataProvider.prototype = {
        constructor : ActionDataProvider,

        loadData : function(from) {
            new Request(this.action, this.params).send(function(data){

            });
        }
    };

    window.AdapTable = function (title, dataLoader) {
        this.title = title;
        this.dataLoader = dataLoader;
        this.defaultCellRenderer = function(rowId, value) {return value;}
...
(function() {
    ...
    Pager.prototype.load = function(pageIdx) {
        ...
        if (objForCurrentPage && objForCurrentPage.length > 0) {
            ...
            if (this.cursor) {
                this.requestParams.cursor = this.cursor;
            }
            var that = this;
